-- ================================================================
-- Vehicle Service Shop — Schema DDL v2 (PostgreSQL 14+)
-- Updated to match current models.py as of 2026-08-13
-- ================================================================
--
-- CHANGES FROM v1 (vehicle_service_shop_schema (2).sql):
--
-- NEW TABLES (3):
--   - user_account          (JWT authentication, roles, links to customer/technician)
--   - diagnostic_template   (reusable diagnostic checklist templates)
--   - diagnostic_template_item (individual items within a template)
--
-- NEW COLUMNS ON EXISTING TABLES (11):
--   - appointment.preferred_time          VARCHAR(10)
--   - visit.walk_in                       BOOLEAN NOT NULL DEFAULT FALSE
--   - line_item.is_complimentary          BOOLEAN NOT NULL DEFAULT FALSE
--   - line_item.warranty_required         BOOLEAN NOT NULL DEFAULT FALSE
--   - work_order.diagnostic_bypassed      BOOLEAN NOT NULL DEFAULT FALSE
--   - work_order.bypass_reason            VARCHAR(500)
--   - part.warranty_required              BOOLEAN NOT NULL DEFAULT FALSE
--   - diagnostic_finding.recommended_service  VARCHAR(500)
--   - diagnostic_finding.is_critical      BOOLEAN NOT NULL DEFAULT FALSE
--
-- ENUM CHANGES:
--   - appointment.status: added 'checked_in' (was: requested, confirmed, cancelled)
--
-- DESIGN DECISIONS (unchanged from v1):
--
-- 1. IDs: UUID with gen_random_uuid() defaults, EXCEPT vehicle.vin
--    which is a natural key (17-char VIN).
--
-- 2. Enums: VARCHAR + CHECK constraint, not native Postgres ENUMs.
--
-- 3. ON DELETE: RESTRICT by default. SET NULL only on genuinely
--    optional forward-pointers.
--
-- 4. Circular FK (bay <-> work_order): bay.current_work_order_id is
--    created as a plain column; the FK is added via ALTER TABLE after
--    work_order is created.
--
-- 5. One-to-one relationships enforced with UNIQUE:
--    work_order.quote_id, invoice.work_order_id,
--    warranty.work_order_id, change_order.finding_id.
--
...
CREATE TABLE part (
    ...
    warranty_required BOOLEAN     NOT NULL DEFAULT FALSE                              -- NEW in v2
);
...
CREATE TABLE appointment (
    ...
    status         VARCHAR(20) NOT NULL DEFAULT 'requested'
        CHECK (status IN ('requested', 'confirmed', 'cancelled', 'checked_in')),       -- CHANGED in v2: added 'checked_in'
    preferred_time VARCHAR(10)                                                          -- NEW in v2
);
...
CREATE TABLE visit (
    ...
    walk_in        BOOLEAN NOT NULL DEFAULT FALSE                                      -- NEW in v2
);
...
CREATE TABLE work_order (
    ...
    diagnostic_bypassed BOOLEAN NOT NULL DEFAULT FALSE,                                    -- NEW in v2
    bypass_reason       VARCHAR(500)                                                       -- NEW in v2
);
...
CREATE TABLE line_item (
    ...
    is_complimentary BOOLEAN NOT NULL DEFAULT FALSE,                                       -- NEW in v2
    warranty_required BOOLEAN NOT NULL DEFAULT FALSE                                       -- NEW in v2
);
...
CREATE TABLE diagnostic_finding (
    ...
    recommended_service VARCHAR(500),                                                      -- NEW in v2
    is_critical         BOOLEAN NOT NULL DEFAULT FALSE                                     -- NEW in v2
);

-- ================================================================
-- NEW in v2 — Diagnostic Templates
-- ================================================================
...
CREATE TABLE diagnostic_template_item (
    item_id     UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    template_id UUID NOT NULL REFERENCES diagnostic_template(template_id) ON DELETE CASCADE,
    description VARCHAR(500) NOT NULL
);

-- ================================================================
-- (continued) Job execution
-- ================================================================
...
CREATE TABLE credit_memo (
    ...
    issued_at        TIMESTAMPTZ,
    -- enforce "exactly one of core_id / part_instance_id is set"
...
CREATE TABLE user_account (
    ...
    tech_id       UUID REFERENCES technician(tech_id) ON DELETE SET NULL
);

-- ================================================================
-- INDEXES
-- ================================================================
-- Postgres does NOT auto-index FK columns (only PKs/UNIQUEs).
...
CREATE INDEX idx_diagnostic_template_item_template ON diagnostic_template_item(template_id);  -- NEW in v2
