...
# =============================================================================
#
# STEP 1 (Auto-generated): The base structure of every model class was generated
#   by the tool `sqlacodegen` which reads an existing PostgreSQL database and
#   outputs Python classes. This includes:
#     - __tablename__
#     - __table_args__  (CheckConstraints, PrimaryKeyConstraints, ForeignKeyConstraints, Indexes)
#     - All mapped_column() definitions (the actual database columns)
#
# STEP 2 (Manually written): Everything else was added by hand afterward:
#     - Imports were adjusted (Base imported from app.database instead of generated)
#     - relationship() declarations on every model (sqlacodegen generates these
#       but they needed renaming from singular to plural and fixing circular refs)
#     - @validates() decorators for application-level validation
#     - @hybrid_property computed fields (total_cost, total_balance, etc.)
#     - Visit.validate_status state-machine transition logic
#     - Section comment headers (SECTION 1, 2, 3, etc.)
#     - Event listeners at the bottom (SECTION 8)
#     - The entire User model (SECTION 9) — not in the original SQL schema
#
# You can tell them apart by this rule of thumb:
#   - If it defines a column or a constraint → GENERATED by sqlacodegen
#   - If it defines behavior (validation, computation, side-effects) → MANUAL
# =============================================================================

# --- Imports (MANUAL — adjusted from sqlacodegen output) ---
...
from app.database import Base  # MANUAL: sqlacodegen generates its own Base; we replaced it with our project's Base

# ================================================================
# SECTION 1: Independent Core Entities (No Foreign Keys)  [MANUAL header]
# ================================================================

class Customer(Base):
    # --- GENERATED by sqlacodegen: table name, constraints, columns ---
...
class Technician(Base):
    ...
    @hybrid_property
    def has_expiring_cert(self) -> bool:
        import datetime
        ...
        try:
            insp = inspect(self)
            if 'certifications' not in insp.unloaded:
...
class Part(Base):
    ...
    warranty_required: Mapped[bool] = mapped_column(Boolean, nullable=False, server_default=text('false'))
...
# ================================================================
# SECTION 2: First-Level Dependent Entities  [MANUAL header]
# ================================================================
...
class Bay(Base):
    ...
    # --- MANUAL: relationships (post_update=True fixes circular FK between Bay ↔ WorkOrder) ---
    appointments: Mapped[list['Appointment']] = relationship('Appointment', back_populates='bay')
    current_work_order: Mapped[Optional['WorkOrder']] = relationship('WorkOrder', foreign_keys=[current_work_order_id], back_populates='current_bays', post_update=True)  # MANUAL: post_update added to break circular dependency
    work_orders: Mapped[list['WorkOrder']] = relationship('WorkOrder', foreign_keys='[WorkOrder.bay_id]', back_populates='bay')


# ================================================================
# SECTION 3: Operations & Front-Office Workflow  [MANUAL header]
# ================================================================
...
class Appointment(Base):
    ...
    __table_args__ = (
        CheckConstraint("status::text = ANY (ARRAY['requested'::character varying, 'confirmed'::character varying, 'cancelled'::character varying, 'checked_in'::character varying]::text[])", name='appointment_status_check'),
    ...
    preferred_time: Mapped[Optional[str]] = mapped_column(String(10))
    ...
    def validate_requested_date(self, key, value):
        ...
        try:
            insp = inspect(self)
            if insp.persistent:
                old_val = insp.attrs.requested_date.loaded_value
                if old_val is not NO_VALUE and old_val == value:
                    return value
        except Exception:
            pass
    ...
    def validate_status(self, key, value):
        if value not in ('requested', 'confirmed', 'cancelled', 'checked_in'):
            raise ValueError(f"Invalid status: '{value}'. Must be 'requested', 'confirmed', 'cancelled', or 'checked_in'.")
        return value
...
class Visit(Base):
    ...
    walk_in: Mapped[bool] = mapped_column(Boolean, nullable=False, server_default=text('false'))
    ...
    # --- MANUAL: validation with state-machine transition enforcement ---
    @validates('status')
    def validate_status(self, key, value):
...
class Quote(Base):
    ...
    def has_work_order(self) -> bool:
        return 'work_order' in self.__dict__ and self.work_order is not None
...
class WorkOrder(Base):
    ...
    diagnostic_bypassed: Mapped[bool] = mapped_column(Boolean, nullable=False, server_default=text('false'))
    bypass_reason: Mapped[Optional[str]] = mapped_column(String(500))
    ...
    # --- MANUAL: computed property (sums all line item prices) ---
    @hybrid_property
    ...
    def has_invoice(self) -> bool:
        ...
        if 'invoice' in instance_state(self).unloaded:
            return False
...
# ================================================================
# SECTION 4: Execution / Line Items  [MANUAL header]
# ================================================================
...
class LineItem(Base):
    ...
    is_complimentary: Mapped[bool] = mapped_column(Boolean, nullable=False, server_default=text('false'))
    warranty_required: Mapped[bool] = mapped_column(Boolean, nullable=False, server_default=text('false'))
...
class DiagnosticFinding(Base):
    ...
    recommended_service: Mapped[Optional[str]] = mapped_column(String(500))
    is_critical: Mapped[bool] = mapped_column(Boolean, server_default=text('false'), nullable=False)
...
class DiagnosticTemplate(Base):
    __tablename__ = 'diagnostic_template'
    __table_args__ = (
        PrimaryKeyConstraint('template_id', name='diagnostic_template_pkey'),
    )
    ...
    # --- MANUAL: relationships ---
    items: Mapped[list['DiagnosticTemplateItem']] = relationship('DiagnosticTemplateItem', back_populates='template', cascade="all, delete-orphan")


class DiagnosticTemplateItem(Base):
    __tablename__ = 'diagnostic_template_item'
    __table_args__ = (
        ForeignKeyConstraint(['template_id'], ['diagnostic_template.template_id'], ondelete='CASCADE', name='diagnostic_template_item_template_id_fkey'),
        PrimaryKeyConstraint('item_id', name='diagnostic_template_item_pkey'),
        Index('idx_diagnostic_template_item_template', 'template_id')
    )
    ...
    # --- MANUAL: relationships ---
    template: Mapped['DiagnosticTemplate'] = relationship('DiagnosticTemplate', back_populates='items')
...
# ================================================================
# SECTION 5: Procurement & Inventory  [MANUAL header]
# ================================================================
...
# ================================================================
# SECTION 6: Billing & Financials  [MANUAL header]
# ================================================================
...
class Invoice(Base):
    ...
    def total_balance(self) -> decimal.Decimal:
        ...
        try:
            insp = inspect(self)
            if 'payments' not in insp.unloaded:
    ...
    def total_balance(cls):
        ...
        paid_subq = (
            select(func.coalesce(func.sum(Payment.amount), decimal.Decimal('0.00')))
            .where(Payment.invoice_id == cls.invoice_id)
...
class StorageCharge(Base):
    ...
    def total_charge(cls):
        ...
        # Uses CURRENT_DATE - start_date which in Postgres returns an integer of days
        days = func.greatest(func.current_date() - cls.start_date, 0)
        return cls.daily_rate * cast(days, Numeric)


# ================================================================
# SECTION 7: Warranties  [MANUAL header]
# ================================================================
...
# ================================================================
# SECTION 8: Event Listeners (Triggers & Side-Effects)  [ENTIRELY MANUAL]
# These are NOT generated by sqlacodegen. They implement business
# side-effects: when a field changes, automatically update related fields.
# ================================================================
...
class CommunicationLog(Base):
    ...
    __table_args__ = (
        ForeignKeyConstraint(['customer_id'], ['customer.customer_id'], ondelete='CASCADE', name='comm_log_customer_id_fkey'),
        PrimaryKeyConstraint('log_id', name='communication_log_pkey'),
        Index('idx_comm_log_customer', 'customer_id')
    )
    ...
    status: Mapped[str] = mapped_column(String(50), nullable=False, server_default=text("'sent'::character varying"))
