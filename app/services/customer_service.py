...
from app.models.models import Customer, Vehicle, Appointment, Visit, Quote, WorkOrder, Invoice, Payment, CommunicationLog
...
class CustomerService:
    ...
    async def create_vehicle(db: AsyncSession, payload: VehicleCreate) -> Vehicle:
        await CustomerService.get_customer(db, payload.customer_id)
    ...
    async def get_timeline_events(db: AsyncSession, customer_id: uuid.UUID) -> list[dict]:
        ...
        for appt in res_appts.scalars().all():
            events.append({
                ...
                "description": f"Appointment scheduled for {appt.scheduled_time.strftime('%Y-%m-%d %H:%M')}",
                "amount": None,
                "type": "appointment",
                "status": appt.status
            })
        ...
        for visit in res_visits.scalars().all():
            events.append({
                "title": f"Visit {visit.status}",
                "date": visit.check_in_time,
                "description": f"Vehicle check-in recorded.",
                "amount": None,
                "type": "check_in",
                "status": visit.status
            })
        ...
        for wo in res_wos.scalars().all():
            events.append({
                ...
                "date": wo.created_at,
                "description": f"Work Order created.",
